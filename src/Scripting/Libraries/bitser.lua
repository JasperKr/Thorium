...
local includeMetatables = false -- togglable with bitser.includeMetatables(false)
local SEEN_LEN = {}
...
local function Buffer_makeBuffer(size)
  if not buf_is_writable then
    if type(writable_buf_size) == "nil" then
...
local function deserialize_value(seen)
  ...
  elseif t == 240 or t == 253 then
    ...
    for _ = 1, len do
      ...
      if not key then
        error("cannot deserialize table with nil key")
      end
  ...
  elseif t == 252 then
    ...
    if not ctype then
      error("cannot deserialize cdata of unknown type")
    end
    local len = deserialize_value(seen)
...
return {
  ...
  dumpSnapFile = function(fname, value)
    serialize(value)
    assert(snap.filesystem.write(fname, ffi.string(buf, buf_pos)))
  end,
  loadSnapFile = function(fname)
    local serializedData, error = snap.filesystem.newFileData(fname)
    ...
    -- so make sure deserialize_value returns before loadSnapFile does
    return value
  end,
